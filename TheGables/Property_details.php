<?php 
 session_start();
       ...
       if($error!=null){
        ...
        {  
          $id=$_GET['id'];
               $query = "select p.* ,cat.category ,h.id as owner_id ,h.name as owner_name ,h.email_address,h.phone_number,m.path from property p left join propertyimage m on m.property_id=p.id join propertycategory cat on p.property_category_id=cat.id join"
                . " homeowner h on h.id=p.homeowner_id where  p.id=".$id ;
        $PSQl = mysqli_query($connection, $query);
        $Property = mysqli_fetch_assoc($PSQl); 

        $query1 = "select * from propertyimage where  property_id=".$id ;
        $PSQl1 = mysqli_query($connection, $query1);
        $images = mysqli_fetch_all($PSQl1); 
  ...
  color: rgb(255, 255, 255, .7);
    ...
    <br>
        ...
        <h2> <?php echo $Property['name']; ?> </h2>
           ...
           $sql3= "select p.id as property_id from Property p where p.id not in ( select property_id from RentalApplication where home_seeker_id=$seekerID) ";
           $result3= mysqli_query($connection, $sql3);
           
           //while($value1= mysqli_fetch_assoc($result3)){
               if(!isset($_GET['requested'])){
?>                       <form method="POST" action="HomeSeeker.php" >
...
?>
        ...
        <form action="Edit_property.php" enctype="multipart/form-data" method="GET" >
            <?php if($_SESSION['role']=='HomeOwner'){?>
              <input type="hidden" <?php echo 'value='.$Property['id']; ?> name="id">
         ...
         <br>
            <div id="Pimg">
                ...
                <?php foreach($images as $image){ ?>
                    <img src="images/<?php echo $image[2];?>" width="100" height="100" alt="Property image 1">
                <?php } ?>
