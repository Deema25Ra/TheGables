...
<script>
  $(document).ready(function() {
    ...
    $('.submit').click(function() {
      ...
      var rental_application_id = $(this).siblings('[name="rental_application_id"]').val();
      var property_id = $(this).siblings('[name="property_id"]').val();
      var status = $(this).val();
      
       $.get('updateStatus.php', {status: status, rental_application_id: rental_application_id}, function(response) {
  
   if (response) {
    $('#status_' + rental_application_id).text(status);
    $('.status_' + property_id).not('#status_' + rental_application_id).text('declined');
  }
});
    });
  });
...
<script>
              ...
              $(document).ready(function(){
                  ...
                  $.post("deleteProperty.php" ,{ id: $(this).val()} ,
             function(data){
               
                 td = "#".concat(JSON.parse(data))
                 $(td).remove()
...
button:active {
      ...
      <td><a href="Property_details.php?id= <?php echo $row["property_id"]; ?>">
               <?php echo $row["property_name"];
                ...
                <td>
                    ...
                    <td class="status_<?php echo $row["property_id"]; ?>" id="status_<?php echo $row["rental_application_id"]; ?>">
  <?php echo $row["application_status"]; ?>
            ...
            <button type='button' class='submit' name='status' value='accepted' style='font-size:70%; color:green'>✔ Accept</button>
            <button type='button' class='submit' name='status' value='declined' style='font-size:65%; color:red'>✘ Decline</button>
                </td>
  ...
  <tr id=<?php echo $row["id"]; ?>>
   ...
   ?> </td>
    ...
    <td>
        ...
        <button name="delete"  class ="delete" value="<?php echo $row["id"]; ?>" style="font-size: 70%; color: black;">Delete</button>
