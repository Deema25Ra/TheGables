     ...
     $connection=mysqli_connect("localhost","root","root","Renting");
        ...
        <main>
         ...
         <div id="top">
            ...
            <pre id="SeekerInfo" title="Your Information!"> 
                 <strong>Name:</strong> <?php echo $row["first_name"].' '.$row["last_name"] ?> 
...
where  home_seeker_id=$seekerID ";
    ...
    <th>Category</th>
     ...
     <th>Status</th>
       ...
       ?>
  <tr>
      <td><a href="Property_details.php?id=<?php echo$value["property_id"];?>&requested=1">
               <?php echo($value["name"]);?></a></td>
      
      <td><?php echo($value["category"]);?> </td>
      <td><?php echo($value["rent_cost"]."/month");?> </td>
    <td><?php echo($value["status"]) ;?></td>
     <?php } ?>
        ...
        <div id="add">
            ...
            <select style="font-family: Papyrus; font-size: 100%"  id="categoryS" name="categoryS"  >
...
where p.id not in (select property_id from RentalApplication where home_seeker_id=$seekerID)";
  ...
  <tr>
    ...
    <td><?php echo($value1["rent_cost"]."/month");?></td>
...
// the id of prop det is not true
    $(document).ready(function () {
        $('#categoryS').change(function () {  // id of select drop down

        
        
            let value = $(this).val();
            let seeker = '<?php echo $seekerID;?>';
            
            $.ajax({
                type: "Post",
                url: "getPropertyByCategory.php",
                dataType:"json",
                data: {CId: value, SId: seeker},
                ...
                success: function (response) {
                   ...
                   $.each(response, function (key, value) {  // value take row by row
                       allprops += '<tr><td class="Property"><a href="Property_details.php?id=' + value.property_id + '">' 
                               + value.name + '</a></td><td>' + value.category 
                               + '</td><td>' + value.rent_cost 
                               + "/month" + '</td><td>' + value.rooms 
                               + '</td><td>' + value.location + '</td>'
                               +'<td><form method="POST" action="HomeSeeker.php">'+'<input type="hidden" name="property_id" value="' + value.property_id + '">'+
                               '<button type="submit" class="ApplyButton" >Apply</button></form></td>'
                               +'</tr>';   
                    });
                    $('#Property_cat').html(allprops);
                }
            });
        });
    });
